...
`article.json` 中的 `publishable` 是投递依据。只有主题聚焦、逻辑连贯、结构清晰和标题质量四个关键终审维度均达到 85 分，且标题 A/B 测试最高分不低于 85，文章才可能进入微信或 QQ 推送；事实核验、篇幅、中文比例、引用和去 AI 味等基础门禁仍须同时通过。`false` 表示文章只能作为资料或人工编辑起点。`visuals.status` 记录配图是否成功，它本身不会决定文字文章能否发布。
